...
bool esValido(const vector<int>& numLetras, int pos) {
...
void maneras(vector<char> &palabra, vector<int> &numLetras , const vector<char>& letras,int m, int n, int k) {
    for (int i = 0; i < m; i++) {
        palabra.push_back(letras[i]);
        ++numLetras[i];

        if (esValido(numLetras,i)) {
            if (k == n - 1) {
                mostrar(palabra, n);
            }
            else {
                maneras(palabra, numLetras, letras, m, n, k + 1);
            }
        }
        //desmarcamos
        --numLetras[i];
...
bool resuelveCaso() {
    ...
    maneras(palabras, numLetras, letras, m, n, 0);
