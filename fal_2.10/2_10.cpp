...
int decreciente(int n, int acumulado, int mult, int der) {
    if (n < 10) {
        if (n >= der) {
    ...
    if (digito >= der) {
        return decreciente(n / 10, acumulado + mult * digito, mult * 10, max(digito, der));
    }
    else {
        return decreciente(n / 10, acumulado, mult, max(digito, der));
    }
}
